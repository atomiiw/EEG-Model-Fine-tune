...
def EA(x, return_matrix=False):
    """
    Euclidean Alignment (EA)
    Returns both aligned data and optionally the transformation matrix.
    """
...
def process_and_replace_loader(loader, ischangechn, dataset, return_ea=False):
    ...
    new_dataset = TensorDataset(torch.from_numpy(processed_data).float(), labels_tensor)
    loader_args = {
    ...
    new_loader = DataLoader(new_dataset, **loader_args)
...
def train_subject(args, subject, seed, device, log_file):
    ...
    # ✅ Save EA matrix for reuse during testing
